...
def multihead_attention(queries,
    ...
    # Linear projections
    Q = tf.layers.dense(queries, num_units, activation=tf.nn.relu)  # todo: num_units=64是啥
...
class MAIM:
    def __init__(self, args, feature_size, run_cnt):
        ...
        if args.is_save is True and os.path.exists(self.save_path) is False:
        ...
        self.temp = 10
        self.lambda_attention = args.lambda_attention
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():
            ...
            self.embeddings = tf.multiply(self.embeddings, self.weights['factor_I'])  # None * M * d
            ...
            # ---------- attention network------------------------------
            self.y_att = self.y_deep  # None * M * output_size
            if self.attention:
                self.attention_mul = tf.reshape(
                    tf.matmul(tf.reshape(self.y_att, shape=[-1, self.output_size]),
                              self.weights['attention_W']),
                    shape=[-1, self.field_size, self.attention_size])  # None * M * as
                self.attention_mul = tf.div(self.attention_mul, self.temp)  # tao

                self.attention_exp = tf.exp(
                    tf.reduce_sum(tf.multiply(self.weights['attention_p'],
                                              tf.nn.relu(self.attention_mul + self.weights['attention_b'])), 2,
                                  keep_dims=True))  # None * M * 1
                ...
                self.attention_out = tf.div(self.attention_exp, self.attention_sum,
                                            name="attention_out")  # None * M * 1
                self.attention_out = tf.nn.dropout(self.attention_out, self.dropout_keep_prob[0])  # dropout
                self.attention_out = tf.multiply(self.attention_out, self.weights['factor_O'])  # None * M * output_size
                self.y_att = tf.multiply(self.y_deep, self.attention_out)  # None * M * output_size

            self.flat_att = tf.reshape(self.y_att, shape=[-1, self.output_size * self.field_size])

            self.out = tf.add(tf.matmul(self.flat_att, self.weights["prediction"]),
            ...
            # l2 regularization on I and O
            if self.attention and self.lambda_attention > 0:
                self.loss += tf.contrib.layers.l2_regularizer(self.lambda_attention)(
                    self.weights['factor_I']) + \
                             tf.contrib.layers.l2_regularizer(self.lambda_attention)(
                                 self.weights['factor_O'])
    ...
    def _init_session(self):
        if self.gpu_memory is not None and 0 < self.gpu_memory < 1:
            gpu_options = tf.GPUOptions(per_process_gpu_memory_fraction=self.gpu_memory)
    ...
    def _initialize_weights(self):
        ...
        # attention layers
        if self.attention:
            glorot = np.sqrt(2.0 / (self.attention_size + self.output_size))
            weights['attention_W'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(self.output_size, self.attention_size)),
                dtype=np.float32, name="attention_W")  # d * A
            weights['attention_b'] = tf.Variable(
                np.random.normal(loc=0, scale=glorot, size=(1, self.attention_size)), dtype=np.float32,
                name="attention_b")  # 1 * A
            weights['attention_p'] = tf.Variable(
                np.random.normal(loc=0, scale=1, size=self.attention_size), dtype=np.float32,
                name="attention_p")  # A

            weights['factor_I'] = tf.Variable(
                np.random.normal(loc=0, scale=1, size=(self.field_size, self.embedding_size)),
                dtype=np.float32,
                name="factor_I")  # f * d

            weights['factor_O'] = tf.Variable(
                np.random.normal(loc=0, scale=1, size=(self.field_size, self.output_size)),
                dtype=np.float32,
                name="factor_O")  # f * d
