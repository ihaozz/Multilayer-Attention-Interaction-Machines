...
def parse_args():
    ...
    parser.add_argument('--attention_size', type=int, default=16, help='attention size')
    parser.add_argument('--lambda_attention', type=float, default=0.0, help='l2 lambda attention')
...
if __name__ == "__main__":
    ...
    for i in range(1, args.run_times + 1):
        test_result, test_loss = _run_(MAIM, args, file_name, i)
        test_auc.append(test_result)
        test_log.append(test_loss)

    avg_auc = sum(test_auc) / len(test_auc)
    avg_log_loss = sum(test_log) / len(test_log)

    print('test_auc', test_auc)
    print('test_log_loss', test_log)
    print('avg_auc', avg_auc)
    print('avg_log_loss', avg_log_loss)
